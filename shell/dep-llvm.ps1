...
if ([string]::IsNullOrWhitespace($llvmInstallDir)) { $llvmInstallDir = Join-Path  $env:LIBRARIES_PATH "$env:HOST_TRIPLET${DirSep}llvm" }
$targetScript = Join-Path $PSScriptRoot "$env:HOST_TRIPLET${DirSep}dep-llvm.ps1"
