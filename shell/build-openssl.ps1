...
if ([string]::IsNullOrWhitespace($gitUrl)) { $gitUrl = "https://github.com/openssl/openssl.git" }
if ([string]::IsNullOrWhitespace($gitBranch)) { $gitBranch = "master" }
if ([string]::IsNullOrWhitespace($opensslLibName)) { $opensslLibName = "openssl" }
if ($isToolchain) {
    if ([string]::IsNullOrWhitespace($opensslInstallDir)) { $opensslInstallDir = Join-Path $env:TARGET_USR_SYSROOT $opensslLibName }
    $targetScript = Join-Path $PSScriptRoot "$env:HOST_TRIPLET${DirSep}$env:TARGET_TRIPLE${DirSep}build-openssl.ps1"
    $currentArch = $env:TARGET_ARCH
    $currentPlatform = $env:TARGET_PLATFORM
}
else {
    if ([string]::IsNullOrWhitespace($opensslInstallDir)) { $opensslInstallDir = Join-Path $env:LIBRARIES_PATH "$env:HOST_TRIPLET${DirSep}$opensslLibName" }
    $targetScript = Join-Path $PSScriptRoot "$env:HOST_TRIPLET${DirSep}build-openssl.ps1"
...
if (Test-Path $targetScript) {
    ...
    $DirParams = 'workspacePath', 'gitUrl', 'gitBranch', 'opensslLibName', 'opensslInstallDir', 'opensslConfigDir', 'targetBuildType'
