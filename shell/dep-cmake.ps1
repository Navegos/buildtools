...
if ([string]::IsNullOrWhitespace($cmakeInstallDir)) { $cmakeInstallDir = Join-Path $env:LIBRARIES_PATH "$env:HOST_TRIPLET${DirSep}cmake" }
$targetScript = Join-Path $PSScriptRoot "$env:HOST_TRIPLET${DirSep}dep-cmake.ps1"
