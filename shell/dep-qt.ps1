...
if ([string]::IsNullOrWhitespace($qtInstallDir)) { $qtInstallDir = Join-Path $env:LIBRARIES_PATH "$env:HOST_TRIPLET${DirSep}qt" }
$targetScript = Join-Path $PSScriptRoot "$env:HOST_TRIPLET${DirSep}dep-qt.ps1"

if (Test-Path $targetScript) {
    Write-Host "[OS/ARCH] $env:HOST_PLATFORM $env:HOST_ARCH detected. Delegating..." -ForegroundColor Cyan

    # 1. Ensure the default path is captured if not explicitly provided by the user
    $targetDirParams = 'qtInstallDir', 'qtVersion'
