...
# Cleanup: Remove empty strings, any path containing $qtroot, and the current target (to avoid dups)
...
Write-Host "[UPDATED] ($TargetScope) '$qtbin' synced in EXTCOMPLIBS_PATH" -ForegroundColor $ScopeColor
...
'@  -replace "VALUE_ROOT_PATH", $qtInstallDir `
    ...
    -replace "VALUE_VERSION", $qtVersion
        ...
        if ($choice -eq 'y' -or $choice -eq 'Y') {
            Write-Host "Executing $qtMachineEnvScript..." -ForegroundColor Yellow
            try {
            ...
            Write-Host "Skipped persistent registry update. You can run it later at: $qtMachineEnvScript" -ForegroundColor Gray
        }
    }
