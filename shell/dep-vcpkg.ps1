...
if ([string]::IsNullOrWhitespace($vcpkgInstallDir)) { $vcpkgInstallDir = Join-Path $env:LIBRARIES_PATH "$env:HOST_TRIPLET${DirSep}vcpkg" }
$targetScript = Join-Path $PSScriptRoot "$env:HOST_TRIPLET${DirSep}dep-vcpkg.ps1"
