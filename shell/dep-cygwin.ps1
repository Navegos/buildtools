...
if ($env:HOST_IS_WINDOWS) {
    if ([string]::IsNullOrWhitespace($cygwinInstallDir)) { $cygwinInstallDir = Join-Path $env:LIBRARIES_PATH "$env:HOST_TRIPLET${DirSep}cygwin" }
    $targetScript = Join-Path $PSScriptRoot "$env:HOST_TRIPLET${DirSep}dep-cygwin.ps1"
}
else {
