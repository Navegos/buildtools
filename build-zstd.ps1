...
if ($IsWindows) {
    $platform = "windows"
    if ([string]::IsNullOrWhitespace($zstdInstallDir)) { $zstdInstallDir = "$env:LIBRARIES_PATH\zstd" }
    $targetScript = Join-Path $PSScriptRoot "$($archFolder)-$($platform)\build-zstd.ps1"
}
elseif ($IsLinux) {
    $platform = "linux"
    if ([string]::IsNullOrWhitespace($zstdInstallDir)) { $zstdInstallDir = "$env:LIBRARIES_PATH/zstd" }
...
if (Test-Path $targetScript) {
    ...
    $DirParams = 'workspacePath', 'gitUrl', 'gitBranch', 'zstdInstallDir'
