...
param (
    ...
    [Parameter(HelpMessage = "Path for Jom installation", Mandatory = $false)]
...
if (-not $archFolder) { Write-Error "Unsupported architecture: $currentArch"; return }

# 2. Platform Detection
if ($IsWindows) {
    $platform = "windows"
    if ([string]::IsNullOrWhitespace($jomInstallDir)) { $jomInstallDir = "$env:LIBRARIES_PATH\jom" }
    $targetScript = Join-Path $PSScriptRoot "$($archFolder)-$($platform)\build-jom.ps1"
} else { Write-Error "Unsupported Operating System."; return }

if (Test-Path $targetScript) {
    Write-Host "[OS/ARCH] $platform $currentArch detected. Delegating..." -ForegroundColor Cyan
    $DirParams = 'workspacePath', 'gitUrl', 'gitBranch', 'jomInstallDir'
    foreach ($ParamName in $DirParams) { if (-not $PSBoundParameters.ContainsKey($ParamName)) { $PSBoundParameters[$ParamName] = Get-Variable -Name $ParamName -ValueOnly } }
    & $targetScript @PSBoundParameters
} else { Write-Error "Platform/Arch script not found: $targetScript" }