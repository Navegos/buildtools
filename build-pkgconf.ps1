...
if ($IsWindows) {
    $platform = "windows"
    if ([string]::IsNullOrWhitespace($pkgconfInstallDir)) { $pkgconfInstallDir = "$env:LIBRARIES_PATH\pkgconf" }
    $targetScript = Join-Path $PSScriptRoot "$($archFolder)-$($platform)\build-pkgconf.ps1"
}
elseif ($IsLinux) {
    $platform = "linux"
    if ([string]::IsNullOrWhitespace($pkgconfInstallDir)) { $pkgconfInstallDir = "$env:LIBRARIES_PATH/pkgconf" }
...
if (Test-Path $targetScript) {
    ...
    $DirParams = 'workspacePath', 'gitUrl', 'gitBranch', 'pkgconfInstallDir'
