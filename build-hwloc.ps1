...
if ($IsWindows) {
    $platform = "windows"
    if ([string]::IsNullOrWhitespace($hwlocInstallDir)) { $hwlocInstallDir = "$env:LIBRARIES_PATH\hwloc" }
    $targetScript = Join-Path $PSScriptRoot "$($archFolder)-$($platform)\build-hwloc.ps1"
}
elseif ($IsLinux) {
    $platform = "linux"
    if ([string]::IsNullOrWhitespace($hwlocInstallDir)) { $hwlocInstallDir = "$env:LIBRARIES_PATH/hwloc" }
...
if (Test-Path $targetScript) {
    ...
    $DirParams = 'workspacePath', 'gitUrl', 'gitBranch', 'hwlocInstallDir'
