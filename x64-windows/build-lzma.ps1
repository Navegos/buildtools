...
$StaticLib = Join-Path $lzmaLibDir ("$lzmaLibName" + "static.lib")
$SharedLib = Join-Path $lzmaLibDir "$lzmaLibName.lib"
$BinaryLib = Join-Path $lzmaBinPath "$lzmaLibName.dll"
...
# Rename bynary lib to lzma.dll to avoid lib not found later on
$BinaryLibPath = Join-Path $lzmaBinPath "liblzma.dll"
$NewBinaryLibName = Join-Path $lzmaBinPath "lzma.dll"
if (Test-Path $BinaryLibPath) {
    Move-Item -Path $BinaryLibPath -Destination $NewBinaryLibName -Force -ErrorAction SilentlyContinue
    Write-Host "Binary library renamed to lzma.dll" -ForegroundColor Gray
}
...
$lzmaabiversion = "VALUE_ABI_VERSION"
$lzmasoversion = "VALUE_SO_VERSION"
$lzmabinary = "VALUE_BINARY"
$lzmashared = "VALUE_SHARED"
$lzmastatic = "VALUE_STATIC"
$lzmalibname = "VALUE_LIB_NAME"
...
$env:LZMA_LIB_NAME = $lzmalibname
$env:LZMA_VERSION = $lzmaversion
$env:LZMA_MAJOR = ([version]$lzmaversion).Major
$env:LZMA_MINOR = ([version]$lzmaversion).Minor
$env:LZMA_PATCH = ([version]$lzmaversion).Patch
$env:LZMA_ABI_VERSION = $lzmaabiversion
$env:LZMA_SO_VERSION = $lzmasoversion
...
'@  -replace "VALUE_ROOT_PATH", $lzmaInstallDir `
    ...
    -replace "VALUE_LIB_NAME", $lzmaLibName `
